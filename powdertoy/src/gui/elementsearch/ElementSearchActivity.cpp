...
ElementSearchActivity::ElementSearchActivity(GameController * gameController, std::vector<Tool*> tools) :
	...
	exit(false)
{
	ui::Label * title = new ui::Label(ui::Point(4, 5), ui::Point(Size.X-8, 0), "Element Search");
...
{ 
    Tool*activeTool = gameController->GetActiveTool(0);                                // 
    ui::Label * title2 = new ui::Label(ui::Point(4, 5), ui::Point(Size.X-8, 20), activeTool->GetDescription());     // 
    title2->SetTextColour(style::Colour::InformationTitle);                             // 
    title2->Appearance.HorizontalAlign = ui::Appearance::AlignLeft;                         // 
    switch (key) 
    { 
    case SDLK_KP_ENTER: 
    case SDLK_RETURN: 
